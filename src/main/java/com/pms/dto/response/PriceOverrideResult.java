 ...
 *
 * <p>🔴 저장된 옵션은 {@code price_source} 가 <b>그대로 AUTO</b>다(D2) — 다음 재계산이 공식값으로 덮는 것이
 * 정상 동작이고, 그것이 「이번 한 번만」의 정의다. {@code market_price} 는 건드리지 않으므로 그 행은 곧바로
 * 「아직 안 밀림」으로 뜬다(D10).</p>
 *
 * @param applied 판매가를 저장한 옵션 수
 * @param skipped 저장하지 않은 옵션 — 직접 지정가(D4) · 판매중 아님. 🔴 화면이 걸렀다고 믿지 않고 서버가 다시
 *                판정한 결과다. ⚠️ 「마켓 식별자 없음」은 여기 들어오지 <b>않는다</b>: 전송에만 필요한 조건이라
 *                아직 등록 전인 셀도 가격은 정할 수 있다
