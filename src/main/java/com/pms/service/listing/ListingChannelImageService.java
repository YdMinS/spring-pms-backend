 ...
 * {@code existsByPlatformProductId} 중복 가드에 걸린다. 그 가드는 "한 쿠팡 상품 = 한 셀" 을 지키는
 * <b>제 일을 하고 있으므로 건드리지 않는다</b> — 대신 <b>이미 연결된 셀</b>을 대상으로 하는 이 경로를
 * 따로 둔다. 이미 연결된 셀에는 애초에 중복 검사가 필요 없다(중복은 이미 해소된 상태다).</p>
 *
