 ...
 * (PLAN 2609_11 D2 개정 2026-09-03). 서버는 받은 코드를 코드 카탈로그
 * ({@link com.pms.domain.CarrierCatalog}, PLAN 2609_37 D1) 화이트리스트로 검증하고 전송하므로
 * 임의 문자열이 쿠팡까지 가지 않는다.
 *
 * <p>⚠️ {@code carrierName} 에는 코드가 들어 있지 않다 — 화면 라벨 {@code 이름 (코드)} 의 괄호는
 * 프론트가 붙인다(PLAN 2609_37 D14).
 *
