 ...
 * live market listings orphaned, so the caller must unlink those channels first → 409 Conflict via the
...
public class MasterProductInUseException extends BusinessException {
    ...
    public MasterProductInUseException(long onMarketCount) {
        super(onMarketCount + "개 채널이 마켓에 등록돼 있어 삭제할 수 없습니다. 먼저 [연결 해제] 후 다시 시도하세요.",
                HttpStatus.CONFLICT);
    }
}
