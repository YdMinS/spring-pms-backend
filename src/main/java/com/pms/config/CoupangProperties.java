...
public class CoupangProperties {
     ...
     * (표시 필터는 orders.orderedAt 기준 — 주문 createdAt 은 저장하지 않음.)
     *
     ...
     *
     * <p>🔴 4.0 이 아니라 3.0 인 이유: {@code callBurst}(1)를 더한 값이 1초 구간의 최대치이므로 4.0 은
     * 정확히 한도 5 에 붙는다 — 여유가 0 이라 쿠팡이 구간을 우리와 조금만 다르게 잘라도 429 다.
     * 2026-09-16 09:15 KST prod 429 당시 우리 간격은 설정대로 250ms 였다(로그 실측). 그 사고의 주범은
     * dev/prod 가 같은 업체코드를 동시에 친 것이지만(→ {@code application-dev.yml}), 한도에 붙어 있던
     * 것도 사실이라 한 칸 내려 여유를 만든다.
     *
     * <p>⚠️ 적응형 자동 상향을 만들지 말 것(D4) — {@code X-CAG-Warnings} 는 관측용이다.
     */
    private double callsPerSecond = 3.0;

    /**
     * 버스트 허용치(토큰 버킷 용량). 쉰 직후의 호출이 기다리지 않게 한다.
     *
     * <p>🔴 <b>{@code callBurst + callsPerSecond < 5} 를 깨지 말 것.</b> 어떤 1초 구간에 나갈 수 있는
     * 최대 호출 수가 바로 이 합이다(쌓여 있던 버스트 + 그 사이 보충량). 쿠팡 한도가 업체코드당
     * 초당 5회다 — <b>같다</b>가 아니라 <b>작다</b>여야 한다(2026-09-16 429). 5 로 두었던 시절 prod
     * 실측이 한 구간에 7건이었다(2026-09-14, 429 는 안 맞았지만 여유가 없었다).
     *
     * <p>1 이어도 <b>단발 호출은 기다리지 않는다</b> — 쉬고 있었으면 토큰이 차 있다. 연달아 쏘는
     * 쪽만 333ms 간격으로 늘어선다. "짧은 작업"과 "긴 작업"을 버킷이 구분하는 것이 아니라,
     * 직전에 쉬었는지만 본다.
     */
    private int callBurst = 1;
    ...
    /**
     * 한 채널의 동기화 락을 강제로 회수하기까지의 시간(분) (FEATURE_2609_48 / D4).
     *
     * <p>쿠팡 읽기 타임아웃 60초 × 왕복 14회라 한 회차가 이론상 10분 넘게 살아 있을 수 있다. 그 정도로
     * 끈 회차는 정상이 아니므로, 이 시간을 넘긴 락은 다음 요청이 뺏는다 — 안 그러면 그 채널은 프로세스가
     * 재시작될 때까지 계속 거절된다.
     *
     * <p>⚠️ 이 값은 "동기화를 얼마나 자주 할 수 있는가" 가 아니다. 그런 제한은 이 번들에 없다(PLAN §번복).
     */
    private int syncLockMaxHoldMinutes = 10;
}
