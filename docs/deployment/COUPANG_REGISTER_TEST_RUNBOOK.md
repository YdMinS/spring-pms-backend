...
| ⑪ | 🟡 **미판정** — 계량 고시 값 형식 (`320g 1개`) | 프론트 101 이 `${계량값} ${수량}개` 로 조합(`composeMeasureNotice` 한 곳). 계량값은 물품 `netContent`+단위(`320g`), 수량은 구성상품 수량 합 | 96 ⑨/97 이 **빈 값**을, 101 이 **틀린 값**(수량만 넣던 `1`)을 막았으나 **쿠팡이 이 형식(특히 `개`)을 받는지는 미판정**. `포장단위별 내용물의 용량(중량), 수량` 같은 계량 고시가 대상 | §5 #10 재시도로 확정. 형식 거부 시 **`composeMeasureNotice` 한 함수만** 고친다(리터럴이 그 밖으로 새지 않게 설계됨). ⚠️ 크기까지 요구하는 농수축산물 키(`…용량(중량),수량,크기`)는 조합이 한 조각 비므로 사용자가 덧붙인다 |
